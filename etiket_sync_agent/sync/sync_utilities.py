...
class sync_utilities:
    @staticmethod
    def create_or_update_dataset(live_mode : bool, s_item : SyncItems, ds_info : dataset_info, dataset_manifest: DatasetManifest):
        '''
        ...
        '''
        dataset_manifest.add_log(f"Creating or updating dataset {ds_info.name} on remote server.")
        
        with get_db_session_context() as session_sync:
            with Session() as session_etiket:
                try :
                    ds = dataset_read(ds_info.datasetUUID)
                    dataset_manifest.add_log(f"Dataset record found on remote server (by uuid).")
                    ...
                    try:
                        ...
                        dataset_manifest.add_log(f"Dataset record found on remote server (by alt_uid), updating uuid to match the one on the remote server.")
                    except DatasetNotFoundException:
                        ...
                        if ds_info.alt_uid is not None:
                            try:
                                ...
                                if ds_local.uuid != s_item.datasetUUID:
                                    ...
                                    dataset_manifest.add_log("Found alt_uid in a dataset on the local server, updating uuid ...")
                            except DatasetNotFoundException:
                                pass
                        ...
                        dataset_manifest.add_log(f"Dataset record created on remote server.")
                ...
                if ds is not None:
                    ...
                    if needs_update:
                        ...
                        dataset_manifest.add_log(f"Dataset record updated on remote server.")
                    else:
                        logger.info("Dataset record found on remote server, no update needed.")
                        dataset_manifest.add_log("Dataset record found on remote server, no update needed.")
                if live_mode:
                    try:
                        ds = dao_dataset.read(s_item.datasetUUID, session=session_etiket)
                        dataset_manifest.add_log(f"Dataset record found on local server (Live Dataset).")
                    except DatasetNotFoundException:
                        ...
                        dataset_manifest.add_log(f"Dataset record created on local server.")
                    ...
                    if needs_update:
                        ...
                        dataset_manifest.add_log(f"Dataset record updated on local server.")
                        logger.info("Dataset record updated on local server.")
                    else:
                        dataset_manifest.add_log(f"Dataset record found on local server, no update needed.")
                        logger.info("Dataset record found on local server, no update needed.")
                                
    @staticmethod
    def upload_xarray(xarray_object : xarray.Dataset, s_item : SyncItems,  f_info : file_info, dataset_manifest: DatasetManifest):
        dataset_manifest.add_log("Converting xarray object to netcdf file")
        try:
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                dataset_manifest.add_log("Conversion successfull")
                f_info.fileType = FileType.HDF5_NETCDF
                sync_utilities.upload_file(file_path, s_item, f_info, dataset_manifest)
        except Exception as e:
            dataset_manifest.add_log(f"Error converting xarray object to netcdf file: {str(e)}")
            dataset_manifest.add_error(e)
            logger.exception(f"Failed to convert xarray object to netcdf file {f_info.name}")
            # fail silently -- errors noted in the manifest.
    
    @staticmethod
    def upload_JSON(content : 'Dict | List | Set | str | int | float', s_item : SyncItems, f_info : file_info, dataset_manifest: DatasetManifest):
        dataset_manifest.add_log("Converting JSON object to file")
        try:
            with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            dataset_manifest.add_log(f"Error converting JSON object to file: {str(e)}")
            dataset_manifest.add_error(e)
            logger.exception(f"Failed to convert JSON object to file {f_info.name}")
            # fail silently -- errors noted in the manifest.
    
    @staticmethod
    def upload_file(file_path, s_item : SyncItems, f_info : file_info, dataset_manifest: DatasetManifest):
        try:
            dataset_manifest.add_log(f"Starting upload process for file {f_info.name}")
            if os.stat(file_path).st_size == 0:
                logger.warning("File %s is empty, skipping.", file_path)
                return
            
            dataset_manifest.add_log(f"Reading file versions for file {f_info.name}")
            r_files, l_files = read_files(s_item.datasetUUID, f_info.name)
            print(r_files, l_files)
            # get checksums
            ...
            if f_info.fileType is FileType.HDF5_NETCDF:
                try:
                    ...
                    dataset_manifest.add_log(f"Could not calculate md5 (NETCDF4) checksum for file {f_info.name}. This file will be considered as a normal H5 file.")
                    f_info.fileType = FileType.HDF5
                    md5_checksum_netcdf4 = md5(file_path)
            
            file_version_id = generate_version_id(f_info.created)
            print("FILE VERSION ID", file_version_id)
            print("R FILES", r_files)
            print("L FILES", l_files)
            ...
            print("MAX VERSION ID", max_version_id)
            ...
            print("LOCAL VERSION", local_version_compatibility, local_version_replace)
            print("REMOTE VERSION", remote_version_compatibility, remote_version_replace)
            print("LAST LOCAL VERSION", last_local_version_compatibility, last_local_version_replace)
            print("LAST REMOTE VERSION", last_remote_version_compatibility, last_remote_version_replace)
            ...
            if (not (local_version_compatibility is FileCompatibility.MISMATCH or remote_version_compatibility is FileCompatibility.MISMATCH) and not 
                ...
                print("NO MISMATCH, using current version", local_version_compatibility, remote_version_compatibility)
                if remote_version is None:
                    dataset_manifest.add_log("No remote version found, creating new file on server")
                    file_create(file_create_data)
                else:
                    dataset_manifest.add_log("Remote version found, no need to create new file on server")
                
                upload_file_to_server(file_path, file_create_data.uuid, file_create_data.version_id, md5_checksum, md5_checksum_netcdf4, remote_version_replace, dataset_manifest)
                
                if local_version is not None and local_version_replace is True:
                    dataset_manifest.add_log("Local version found, replacing local file, as it is not immutable and the content is different.")
                    replace_local_file(s_item.datasetUUID, file_path,local_version, dataset_manifest)
                ...
                (last_local_version_compatibility is not FileCompatibility.MISMATCH and last_remote_version_compatibility is not FileCompatibility.MISMATCH)):
                    ...
                    print("USING LAST VERSION", last_local_version_compatibility, last_remote_version_compatibility)
                    if remote_last_version is None:
                        dataset_manifest.add_log("No remote version found, creating new file on server (last version present of the local file).")
                        file_create(file_create_data)
                    else:
                        dataset_manifest.add_log("Remote version found, no need to create new file on server (last version present of the local file).")
                    
                    upload_file_to_server(file_path, file_create_data.uuid, file_create_data.version_id, md5_checksum, md5_checksum_netcdf4, last_remote_version_replace, dataset_manifest)
                    
                    if local_last_version is not None and last_local_version_replace is True:
                        dataset_manifest.add_log("Local version found, replacing local file, as it is not immutable and the content is different.")
                        replace_local_file(s_item.datasetUUID, file_path, local_last_version, dataset_manifest)
            else:
                print("CREATING NEW FILE", last_local_version_compatibility, last_remote_version_compatibility)
                dataset_manifest.add_log("File is not compatible with the existing file versions, creating new file on server.")
                file_create_data.version_id = generate_version_id(datetime.datetime.now()) # TO Discuss, should the be the last modified time?
                file_create(file_create_data)
                upload_file_to_server(file_path, file_create_data.uuid, file_create_data.version_id, md5_checksum, md5_checksum_netcdf4, False, dataset_manifest)
        except CONNECTION_ERRORS as e:
            dataset_manifest.add_log(f"Connection error: {str(e)}, stopping now.")
            raise e
        except Exception as e:
            dataset_manifest.add_log(f"Error uploading file to server: {str(e)}")
            dataset_manifest.add_error(e)
...
def upload_file_to_server(file_path : str, file_uuid : uuid.UUID, file_version_id : int, md5_checksum : Any, md5_checksum_netcdf4 : Optional[Any], replace_file : bool, dataset_manifest: DatasetManifest):
    """
    ...
    """
    dataset_manifest.add_log(f"Uploading file to server with version id {file_version_id}")
    try:
        ...
        if file.status == FileStatusRem.secured and replace_file:
            dataset_manifest.add_log("File is secured on server, but replace is requested, overwriting file on server.")
        
        if should_upload:
            dataset_manifest.add_log(f"Uploading file to server (status: {file.status}, replace: {replace_file})")
            ...
            dataset_manifest.add_log("File upload completed successfully")
        else:
            dataset_manifest.add_log("File already secured on server, skipping upload")
            
    except Exception as e:
        dataset_manifest.add_log(f"Error uploading file to server: {str(e)}")
        logger.error(f"Failed to upload file {file_uuid} version {file_version_id}: {str(e)}")
        raise e

def replace_local_file(dataset_uuid : uuid.UUID, file_path : str, file : FileReadLocal, dataset_manifest: DatasetManifest):
    """
        ...
        with Session() as session:
            ...
            if file.local_path is not None and os.path.exists(file.local_path):
                try:
                    ...
                    dataset_manifest.add_log(f"Removed old local file: {file.local_path}")
                except OSError as e:
                    dataset_manifest.add_log(f"Warning: Could not remove old file {file.local_path}: {str(e)}")
                    logger.warning(f"Could not remove old file {file.local_path}: {str(e)}")
            ...
            dataset_manifest.add_log(f"Copied new file to: {local_path}")
            ...
            dataset_manifest.add_log("Local file replacement completed successfully")
            
    except Exception as e:
        error_msg = f"Failed to replace local file for {file.uuid}: {str(e)}"
        dataset_manifest.add_log(f"Error: {error_msg}")
